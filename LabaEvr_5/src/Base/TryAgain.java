        ...
        while (counter<k) {
            ...
            for (int i = 0; i < Z; i++) {
               ...
               // ArrayList<ArrayList<Integer>> childrenM = new ArrayList<>();
                ...
                fileBuild.append(FenotypeWriter(fenotypeBuild(O.get(0), ParentsGenes.get(pairs.get(i).get(0)), genes)));
                fileBuild.append("Родитель 2: " + ParentsGenes.get(pairs.get(i).get(1)) + "\n");
                fileBuild.append(FenotypeWriter(fenotypeBuild(O.get(0), ParentsGenes.get(pairs.get(i).get(1)), genes)));                for (int j = 0; j < T; j++) {
                ...
                for(int l = 0;l<2;l++) {
                    ...
                    ArrayList<ArrayList<Integer>> Fenotype = fenotypeBuild(O.get(0), ParentsGenes.get(pairs.get(i).get(l)), genes);
                    //fileBuild.append(FenotypeWriter(Fenotype));
                    max = -1;
                    for(int j = 0;j<Fenotype.size();j++){
                        if(SumStroke(Fenotype.get(j))>max) max = SumStroke(Fenotype.get(j));
            ...
            fileBuild.append("max: "+MAX.toString());
